...
def process_video(video_path):
    ...
    try:
        ...
        return call_gpt(frames_base64, video_prefix)
    except Exception as e:
        print(f"Failed processing {video_path}: {e}")
        return video_prefix, "error"


def process_video_threaded(file_path, result_dict):
    video_key, score = process_video(file_path)
    result_dict[video_key] = score


def process_folder(input_video_folder, output_json_file, num_workers):
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for f in tqdm(
            ...
            futures.append(
                executor.submit(process_video_threaded, full_path, result_dict)
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--output_json_file",
        type=str,
        default="demo_result/model_name_output_json/naturalscore.json",
    )
...
def main():
    ...
    process_folder(input_video_folder, output_json_file, num_workers)
